 ...
 * ──────────────────────────────────────────────────────
 * CORREÇÕES APLICADAS:
 *  1. Redirect MP: window.location.href sem setTimeout — elimina sobreposição de CSP
 *  2. Prioridade correta: checkoutUrl > initPoint (respeita lógica TEST vs produção do backend)
 *  3. window.open do WhatsApp (plano Pro) substituído por window.location.href — evita
 *     contaminação de contexto de segurança no Chrome/Edge
 *  4. useRef guard contra double-redirect
 */
...
export default function CheckoutPage() {
  ...
  // Plano Pro: usa location.href em vez de window.open
  // window.open contamina o contexto de segurança da janela atual,
  // corrompendo o nonce CSP do MP em redirects subsequentes no Chrome/Edge.
  useEffect(() => {
    if (planId === "pro") {
  ...
  async function handleSubmit(e) {
    ...
    // Mercado Pago
    // CRITICO: sem setTimeout — qualquer delay mantem o contexto React ativo
    // enquanto o browser comeca a carregar mercadopago.com, causando sobreposicao
    // de CSP e corrompendo o nonce do MP.
    // checkoutUrl ja respeita TEST vs producao no backend (sandbox_init_point vs init_point).
                ...
                {status === "loading" || status === "redirecting"
                  ...
                  : (
                    <>
                      ...
                      Finalizar Pedido &mdash; {plan.priceLabel}
                    </>
                  )
                }
              </button>
...
const checkoutCSS = `
  ...
  .co-grid { position:fixed;inset:0;z-index:0;pointer-events:none;background-image:linear-gradient(rgba(201,168,76,.025) 1px,transparent 1px),linear-gradient(90deg,rgba(201,168,76,.025) 1px,transparent 1px);background-size:70px 70px;mask-image:radial-gradient(ellipse 80% 70% at 50% 0%,black 40%,transparent 100%);-webkit-mask-image:radial-gradient(ellipse 80% 70% at 50% 0%,black 40%,transparent 100%); }
  ...
  .co-wrapper { position:relative;z-index:10;display:grid;grid-template-columns:1fr;width:100%;max-width:900px;background:rgba(10,10,10,.9);backdrop-filter:blur(24px) saturate(160%);-webkit-backdrop-filter:blur(24px) saturate(160%);border:1px solid var(--glass-border);border-radius:var(--radius-xl);overflow:hidden;animation:co-in .5s cubic-bezier(.4,0,.2,1); }
  ...
  .co-summary { padding:2.5rem;background:rgba(201,168,76,.04);border-bottom:1px solid var(--border);position:relative; }
  @media(min-width:768px){ .co-summary{ border-bottom:none;border-right:1px solid var(--border); } }
  .co-back { display:inline-flex;align-items:center;gap:.4rem;font-size:.78rem;color:var(--text-muted);text-decoration:none;margin-bottom:1.75rem;transition:color var(--transition); }
  .co-back:hover { color:var(--gold-2); }
  .co-logo { font-family:var(--font-title);font-size:1.5rem;font-weight:800;letter-spacing:-.04em;background:linear-gradient(90deg,#8a6d28 0%,#e2c97e 50%,#8a6d28 100%);background-size:200% auto;-webkit-background-clip:text;-webkit-text-fill-color:transparent;background-clip:text;animation:co-shimmer 4s linear infinite;margin-bottom:1.5rem; }
  @keyframes co-shimmer { from{background-position:-200% center} to{background-position:200% center} }
  .co-plan-badge { display:inline-flex;align-items:center;gap:.4rem;padding:.3rem .875rem;background:rgba(201,168,76,.1);border:1px solid var(--border);border-radius:100px;font-size:.72rem;font-weight:600;color:var(--gold-3);letter-spacing:.08em;text-transform:uppercase;margin-bottom:1rem; }
  .co-plan-badge-dot { width:6px;height:6px;border-radius:50%;background:var(--gold-2);animation:co-blink 2s ease-in-out infinite; }
  @keyframes co-blink { 0%,100%{opacity:1} 50%{opacity:.3} }
  .co-plan-price { font-family:var(--font-title);font-size:1.8rem;font-weight:800;letter-spacing:-.04em;background:var(--accent-gradient);-webkit-background-clip:text;-webkit-text-fill-color:transparent;background-clip:text;margin-bottom:.4rem; }
  ...
  .co-badge-item { display:flex;align-items:center;gap:.625rem;font-size:.8rem;color:var(--text-muted);font-weight:300; }
  ...
  .co-field input { background:rgba(255,255,255,.03);border:1px solid var(--border);border-radius:var(--radius);padding:.8rem 1rem;font-family:var(--font-body);font-size:.9rem;color:var(--text);outline:none;transition:border-color var(--transition),box-shadow var(--transition),background var(--transition);width:100%; }
  ...
  .co-submit { display:flex;align-items:center;justify-content:center;gap:.625rem;width:100%;padding:1rem;margin-top:.25rem;background:var(--accent-gradient);border:none;border-radius:var(--radius);font-family:var(--font-body);font-size:.95rem;font-weight:700;color:#080808;cursor:pointer;box-shadow:0 4px 20px rgba(201,168,76,.28);transition:transform var(--transition),box-shadow var(--transition),opacity var(--transition); }
